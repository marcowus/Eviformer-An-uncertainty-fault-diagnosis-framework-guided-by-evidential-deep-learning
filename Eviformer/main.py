...
def main():
    ...
    if args.examples:
        ...
        num_classes = 6 #need to change

        ## change the model
        # model = LeNet(dropout=args.dropout)
        # model = vit_middle_patch16(data_size=1024, in_c=1, num_cls=10, h_args=[256, 128, 64, 32])  #这里类别需要更改
        model = mcswint(in_channel=1, out_channel=4)
        ...
        exp_lr_scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=7, gamma=0.5) #tep_size=7, gamma=0.5
        ...
        device = get_device()
        # model = LeNet()
        # model = vit_middle_patch16(data_size=1024, in_c=1, num_cls=4, h_args=[256, 128, 64, 32])
        model = vit_middle_patch16()
        model = model.to(device)
