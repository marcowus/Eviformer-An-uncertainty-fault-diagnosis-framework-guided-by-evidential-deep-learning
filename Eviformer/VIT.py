...
#使patch有重叠
    def __init__(self, data_size, in_c, patch_size, norm_layer, overlap):
        super().__init__()
        self.data_size = data_size
        self.patch_size = patch_size
        self.overlap = overlap
        self.stride = patch_size - overlap  # Calculate stride as patch_size minus overlap
        self.grid_size = (data_size - patch_size) // self.stride + 1  # Recalculate grid_size based on new stride
        self.num_patches = self.grid_size
        self.embed_dim = in_c * patch_size
        self.projection = nn.Conv1d(in_c, self.embed_dim, kernel_size=patch_size, stride=self.stride)
        self.norm = norm_layer(self.embed_dim) if norm_layer else nn.Identity()
        ##
        self.Batch = nn.BatchNorm1d(497) ###overlap为30时，497;overlap=0,32
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, data_size, in_c, num_cls,
        ...
        self.patch_embed = PatchEmbed(data_size=data_size,
                                      ...
                                      overlap=30) ###
        self.num_patches = self.patch_embed.grid_size ###
...
cfgs = {'base': [3, 4, 32],  #[12, 8, 16] 3 4 32
        "middle_1": [12, 16, 16],
        'middle_2': [12, 16, 32],
        'large_1': [24, 16, 16],
        'large_2': [24, 16, 32]}
...
class vit_middle_patch16(nn.Module):
    def __init__(self, data_size, in_c, num_cls, h_args, cfgs=cfgs['base']):
        super(vit_middle_patch16, self).__init__()
        self.model = VisionTransformer(data_size=data_size,
                                       in_c=in_c,
                                       num_cls=num_cls,
                                       h_args=h_args,
                                       cfgs=cfgs)
